...
async function saveMessageToLocalDB(msg) {
    ...
    if (!textBody) return; // Se n├úo tem texto leg├¡vel, ignoramos pro relat├│rio
...
async function startSession(sessionId) {
    ...
    // Reserva a sess├úo na mem├│ria com status INITIALIZING para que os pings n├úo achem que est├í offline
    ...
    const sock = makeWASocket({
        ...
        syncFullHistory: false, // Historicos GIGANTES fazem o celular falhar ("N├úo foi poss├¡vel concluir a sincroniza├º├úo")
    ...
    // Eventos de hist├│rico retroativo (WhatsApp envia logo que conecta um novo dispositivo)
    sock.ev.on('messaging-history.set', async ({ messages, contacts, chats, isLatest }) => {
        console.log(`[WA Baileys] Recebendo lote de hist├│rico! Total de msgs: ${messages.length}`);
        ...
        if (isLatest) {
            ...
            // Se o celular abortar a nuvem ou demorar mais de 15s pra mandar o pr├│x bloco, consideramos finalizado pacientemente.
            syncTimeout = setTimeout(() => {
                console.log(`[WA Baileys] Tempo esgotado para nova remessa. Abortando Sincroniza├º├úo graciosamente.`);
                sessionData.isSyncing = false;
            }, 15000);
        }
    });
...
/**
 * Puxa hist├│rico do SQLite local e pede a OpenAI para lapidar em JSON para 1 ├ÜNICO chat
 */
async function fetchAndAnalyzeHistory(sessionId, contactPhoneOrJid, dataIni, dataFim) {
    if (!openai && process.env.OPENAI_API_KEY) {
        openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
    }
    if (!openai) throw new Error('OPENAI_API_KEY n├úo configurada no backend!');
    ...
    if (!history || history.length === 0) {
        return { success: false, error: 'Sem hist├│rico capturado pelo Baileys no banco local para este per├¡odo.' };
    }
    ...
    const prompt = `
    Abaixo, voc├¬ tem um hist├│rico bruto de conversa de WhatsApp extra├¡do de um servidor web.
    Sua miss├úo ├® gerar um relat├│rio de conversas agrupado POR DIA (apenas as datas presentes no chat), em formato JSON.
    ...
    Para cada data, defina um objeto com as seguintes chaves:
      ...
      - ultima_mensagem_resumo: Uma frase que represente bem o final da conversa ou a ├║ltima mensagem enviada/recebida no dia.

    Apenas me devolva o JSON sem introdu├º├úo ou contra as tags markdown extras (Apenas o conte├║do bruto).
    ...
    try {
        ...
        throw new Error('Erro na integra├º├úo com OpenAI: ' + e.message);
    }
}
...
async function fetchAndAnalyzeAllHistory(sessionId, dataIni, dataFim, onProgress = null) {
    ...
    if (!history || history.length === 0) {
        return { success: false, error: 'Sem mensagens no banco local para este per├¡odo.' };
    }
    ...
    for (const jid of jids) {
        ...
        const prompt = `
        Abaixo, voc├¬ tem um hist├│rico de conversa bruta originada de ${isGroup ? 'um GRUPO de WhatsApp' : 'um CONTATO/CLIENTE de WhatsApp'}.
        O identificador do chat ├®: "${jid.split('@')[0]}"
        Sua miss├úo ├® gerar um array de objetos JSON que represente o dia-a-dia da conversa. O Array n├úo pode estar contido em objeto raiz.
        Cada objeto deve obrigatoriamente ter:
          - "date": String no formato YYYY-MM-DD daquelas mensagens.
          - "contato": ${isGroup
                ? `Tente DESCOBRIR O NOME DESSE GRUPO pelo contexto das mensagens. Se descobrir o nome real, escreva-o e adicione (Grupo). Se for imposs├¡vel achar o nome no meio da conversa, devolva apenas "Grupo ${jid.split('@')[0]}".`
                : `Tente DESCOBRIR O NOME DA PESSOA lendo as mensagens (seja porque o atendente a chamou pelo nome ou ela mesma se apresentou). Se achar, devolva apenas o Nome Real com a primeira letra mai├║scula (ex: "Carlos"). Somente se for IMPOSS├ìVEL achar qualquer nome no texto inteiro, devolva o telefone original "${jid.split('@')[0]}".`
            }
          - "numMensagens": Inteiro representando o n├║mero de bal├Áes de mensagens lidos nesse dia.
          - "horaInicio": String no formato HH:MM (ex: "10:00") correspondente ao hor├írio da PRIMEIRA mensagem deste dia.
          - "horaFim": String no formato HH:MM (ex: "17:30") correspondente ao hor├írio da ├ÜLTIMA mensagem deste dia.
          - "ultimaMensagem": Responda OBRIGATORIAMENTE um resumo de 10-40 palavras sobre os principais assuntos negociados/tratados no dia. OBRIGATORIAMENTE inicie o texto deste resumo citando quem estava na conversa no formato "Participantes: [nome das pessoas ou n├║meros] - " seguido do resumo.
...
async function sendMessageDirect(sessionId, number, text) {
    ...
    if (!sessionData || !sessionData.sock) {
        throw new Error('Sess├úo Baileys n├úo iniciada. Use POST /api/wa/connect primeiro!');
    }

    if (sessionData.status !== 'CONNECTED') {
        throw new Error(`Sess├úo ainda n├úo conectada. Status atual: ${sessionData.status}. Aguarde ou escaneie o QR Code.`);
    }

    // Formata o JID (n├║mero -> n├║mero@s.whatsapp.net)
    ...
    try {
        ...
        console.log(`[sendMessageDirect] Ô£à Mensagem enviada! messageId: ${sent?.key?.id}`);
        return { success: true, jid: jidTarget, messageId: sent?.key?.id };
    } catch (e) {
        console.error(`[sendMessageDirect] ÔØî Erro ao enviar:`, e.message);
